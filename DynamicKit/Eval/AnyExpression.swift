...
public struct AnyExpression: CustomStringConvertible {
    ...
    internal let describer: () -> String
    internal let evaluator: () throws -> Any
    ...
    static func stringify(_ value: Any) -> String {
        ...
        case is Any.Type:
            let typeName = "\(value)"
                ...
                // Fix mangled internal class names on Swift 4 and earlier
...
internal extension AnyExpression {
    // Value storage
    final class NanBox {
        internal static let mask = (-Double.nan).bitPattern
        internal static let indexOffset = 4
        internal static let nilBits = bitPattern(for: -1)
        internal static let falseBits = bitPattern(for: -2)
        internal static let trueBits = bitPattern(for: -3)

        internal static func bitPattern(for index: Int) -> UInt64 {
...
extension ClosedRange: _Range {
    internal func slice(of array: _Array, for symbol: Expression.Symbol) throws -> ArraySlice<Any> {
    ...
    internal func slice(of string: _String, for symbol: Expression.Symbol) throws -> Substring {
        let substring = string.substring
        switch self {
...
internal protocol _Array {
    var values: [Any] { get }
}

internal protocol _SwiftArray: _Array {
    static func cast(_ value: Any) -> Any?
}

extension Array: _SwiftArray {
    internal var values: [Any] {
        return self
    }

    internal static func cast(_ value: Any) -> Any? {
...
internal protocol _Dictionary {
    func value(for key: Any) -> Any?
}

extension Dictionary: _Dictionary {
    internal func value(for key: Any) -> Any? {
...
extension Optional: _Optional {
    internal var value: Any? { return self }
    internal static var wrappedType: Any.Type { return Wrapped.self }
}
